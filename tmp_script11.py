...
def map_radius(raw):
    return clamp(raw * 0.6, 0.12, 0.92)
...
def convert_pole_to_section(theta, r):
    ...
    k1 = clamp(k1, 0.0, 1.0)
    k2 = clamp(map_radius(r)/0.98, 0.0, 1.0)
...
def convert_to_cube(idxA, idxB):
    ...
    for corner in range(8):
        ...
        for theta, r in base:
            ...
            r = map_radius(r * (1.0 + (kRadiusSwing if z else -kRadiusSwing)))
...
def decode_section(sec, sample_rate=48000.0):
    ...
    b2 = -(R*R)
    alpha = (1.0 - R)
    a0 = alpha * sec['gain'] * 1.6
    a1 = 0.0
    a2 = -alpha * sec['gain'] * 1.6
