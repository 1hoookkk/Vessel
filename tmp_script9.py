...
def load_polar(idx):
    arr = shapes[idx]
    # interpret as theta, r pairs swapped
...
def probe_avg(mode_idx=0):
    a,b=pairs[mode_idx]
    cube=convert_to_cube(a,b)
    frame=interpolate_cube(cube,0.5,0.5,0.5)
    coeffs=[decode_section(sec) for sec in frame]
...
for mode_idx in range(len(pairs)):
    avg,mx,mn=probe_avg(mode_idx)
    print('mode',mode_idx,'avg',avg,'max',mx,'min',mn)
