...
def decode_section(sec, sample_rate=48000.0):
    # modified: gentler R curve + normalization at center
    R = (sec['k2']**0.85) * 0.92 + 0.04
    R = clamp(R, 0.12, 0.97)
    ...
    a1 = 0.0
    a2 = -alpha * sec['gain']
    # normalize to ~0dB at centre
    Hc = freq_response((a0,a1,a2,b1,b2), theta)
    mag = abs(Hc)
    if mag > 1e-6:
        scale = 1.0/mag
        a0 *= scale; a2 *= scale
...
def cascade_mag(mode_idx=0, morph=0.5, freq=0.5, trans=0.5, sr=48000.0):
    a,b = pairs[mode_idx]
    cube = convert_to_cube(a,b)
    frame = interpolate_cube(cube, morph,freq,trans)
    coeffs=[decode_section(sec, sr) for sec in frame]
    import numpy as np
...
for mode_idx in range(len(pairs)):
    freqs, mags = cascade_mag(mode_idx=mode_idx, morph=0.5, freq=0.5, trans=0.5)
    print('mode', mode_idx, 'avg', sum(mags)/len(mags), 'max', max(mags), 'min', min(mags))
