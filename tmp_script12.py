...
def convert_pole_to_section(theta, r):
    r = clamp(r * 0.6, 0.1, 0.95)
...
def convert_to_cube(idxA, idxB):
    ...
    for corner in range(8):
        ...
        for theta, r in base:
            ...
            r = r * (1.0 + (kRadiusSwing if z else -kRadiusSwing))
            frame.append(convert_pole_to_section(theta, r))
        cube.append(frame)
    return cube
...
def interpolate_cube(cube,x,y,z):
    ...
    for s in range(6):
        ...
        def tri(vals):
            ...
            c00=lerp(c000,c100,x); c10=lerp(c010,c110,x)
            c01=lerp(c001,c101,x); c11=lerp(c011,c111,x)
            c0=lerp(c00,c10,y); c1=lerp(c01,c11,y)
            return lerp(c0,c1,z)
        k1=tri(tuple(k1_vals)); k2=tri(tuple(k2_vals)); g=tri(tuple(g_vals))
...
def probe(mode_idx=2, sections=6):
    a,b=pairs[mode_idx]
    cube=convert_to_cube(a,b)
    frame=interpolate_cube(cube,0.5,0.5,0.5)
    coeffs=[decode_section(sec) for sec in frame][:sections]
    probe=[80,200,500,1200,2400,5000,10000]
    mags=[]
    for f in probe:
        omega=2*math.pi*f/48000.0
        H=1+0j
        for c in coeffs:
