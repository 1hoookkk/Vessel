...
def probe_avg(mode_idx=0, morph=0.5, freq=0.5, trans=0.5, sr=48000.0):
    a,b = pairs[mode_idx]
    ...
    return sum(mags)/len(mags), max(mags), min(mags), mags

for mode_idx in range(len(pairs)):
    avg,mx,mn,mags=probe_avg(mode_idx=mode_idx,morph=0.5,freq=0.5,trans=0.5)
    print('mode',mode_idx,'probe avg',avg,'max',mx,'min',mn)
